...
#include <time.h>  
#include <sys/time.h>  
#include <stdint.h>  // Include this to use uint64_t  
...
void GraphBFS(int *graph, int *visited, int start) {  
    ...
    uint64_t size = (uint64_t)N * (uint64_t)N * (uint64_t)sizeof(int);  
...
int main() {  
    ...
    // Calculate the size of the graph in bytes  
    uint64_t size = (uint64_t)GRAPH_SIZE * (uint64_t)GRAPH_SIZE * (uint64_t)sizeof(int);  
    ...
    int *graphCopy = (int *)malloc(size);  
    int *visitedCopy = (int *)calloc(GRAPH_SIZE, sizeof(int));  
    memcpy(graphCopy, graph, size);  
